<?php
/*
Plugin Name: Bunched Text Reader
...
add_filter('the_content', 'MBK_Bunched_Text_Reader_Plugin');
function MBK_Bunched_Text_Reader_Plugin($content) {
    $content .= file_get_contents (plugin_dir_path( __FILE__ ) . "page.txt");
    return $content;
}


add_action('wp_enqueue_scripts', 'pwwp_enqueue_bootstrap4_');
function pwwp_enqueue_bootstrap4_() {
    ...
    wp_enqueue_style('main-styles-bunched', plugins_url( 'css/style.css' , __FILE__ ), array(), rand(), false);
	wp_register_style('fontawesome1', "https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css", array());
    wp_enqueue_style('fontawesome1');
}

function bunched_text_reader_scripts() {
    wp_enqueue_script( 'js', plugins_url( 'js/demo.js?x=' . rand(), __FILE__ ), array(), null, true );
    wp_localize_script( 'js', 'frontend_ajax_object',
...
add_action( 'wp_enqueue_scripts', 'bunched_text_reader_scripts' );
